...
import { View, Text, TouchableOpacity, ScrollView, StyleSheet, Alert, ActivityIndicator, Modal, FlatList, TextInput } from 'react-native';
import { router, useLocalSearchParams } from 'expo-router';
import { StatusBar } from 'expo-status-bar';
import { allocateToDealer, removeDealerAllocation, getDealerAllocations, LocationAllocation } from '@/services/locationAllocationService';
import { getDistricts, District } from '@/services/locationService';
import { getUser, User } from '@/services/authService';
import { useTheme } from '@/contexts/ThemeContext';
import { Colors, Fonts } from '@/constants/theme';
import { HeaderWithMenu } from '@/components/HeaderWithMenu';

export default function AllocateLocationToDealerScreen() {
  const { dealerId, dealerName } = useLocalSearchParams<{ dealerId: string; dealerName: string }>();
  ...
  const loadUser = async () => {
    try {
      ...
      setCurrentUser(user);
      if (user?.role !== 'admin') {
        Alert.alert('Access Denied', 'Only admins can allocate locations.');
        router.back();
      }
    } catch (error) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [districtsData, allocationsData] = await Promise.all([
        getDistricts(),
        dealerId ? getDealerAllocations(dealerId) : Promise.resolve([])
      ]);
  ...
  const handleAllocate = async () => {
    if (!selectedDistrict || !dealerId) return;
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await removeDealerAllocation(allocationId);
  ...
  const filteredDistricts = districts.filter(district =>
    district.districtName.toLowerCase().includes(districtSearchQuery.toLowerCase())
  );

  const filteredTalukas = selectedDistrict
    ? selectedDistrict.talukas.filter(taluka =>
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={[styles.scrollView, { backgroundColor: colors.background }]} contentContainerStyle={styles.scrollContent}>
        <View style={[styles.infoCard, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}>
          <Text style={[styles.infoText, { color: colors.text }]}>Dealer: {dealerName || 'Unknown'}</Text>
        </View>
          ...
          <View style={[styles.modalContent, { backgroundColor: colors.cardBackground }]}>
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={true}>
              ...
              <View style={styles.section}>
                ...
                />
                <View style={[styles.districtListContainer, { backgroundColor: colors.background, borderColor: colors.border }]}>
                  <ScrollView style={styles.districtList} nestedScrollEnabled showsVerticalScrollIndicator={true}>
                      ...
                      filteredDistricts.map((district) => (
                        ...
                        >
                          <Text style={[
                            ...
                            { color: colors.text },
                            selectedDistrict?.districtCode === district.districtCode && { color: colors.primary, fontFamily: Fonts.semiBold }
                          ]}>
              ...
              {/* Taluka Selection */}
              {allocationScope === 'specific-talukas' && selectedDistrict && (
                <View style={styles.section}>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  { 
                    ...
                    opacity: submitting || !selectedDistrict ? 0.6 : 1 
                  }
                ]}
